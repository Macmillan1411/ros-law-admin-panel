...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username='editor').exists():
            ...
            self.stdout.write(self.style.SUCCESS(f'Created editor user: {editor.username}'))
        ...
        if not User.objects.filter(username='moderator').exists():
            ...
            self.stdout.write(self.style.SUCCESS(f'Created moderator user: {moderator.username}'))
        ...
        # Create chapters
        chapters_data = [
            {'title': 'Глава 1', 'description': 'Введение в законодательство', 'order': 1},
        ...
        sections_data = [
            {'title': 'Раздел 1.1', 'description': 'Основы законодательства', 'order': 1, 'chapter': chapters[0]},
            {'title': 'Раздел 1.2', 'description': 'Конституционное право', 'order': 2, 'chapter': chapters[0]},
            {'title': 'Раздел 1.3', 'description': 'Международное право', 'order': 3, 'chapter': chapters[0]},
            {'title': 'Раздел 1.4', 'description': 'Нормативные акты', 'order': 4, 'chapter': chapters[0]},
            {'title': 'Раздел 1.5', 'description': 'Правовая система РФ', 'order': 5, 'chapter': chapters[0]},
            {'title': 'Раздел 1.6', 'description': 'Судебная система', 'order': 6, 'chapter': chapters[0]},
        ]
        ...
        subsections_data = [
            {'title': 'Подраздел 1.1.1', 'description': 'Структура законодательства', 'order': 1, 'section': sections[0]},
            {'title': 'Подраздел 1.1.2', 'description': 'Принципы права', 'order': 2, 'section': sections[0]},
            {'title': 'Подраздел 1.1.3', 'description': 'Правовые нормы', 'order': 3, 'section': sections[0]},
            {'title': 'Подраздел 1.1.4', 'description': 'Источники права', 'order': 4, 'section': sections[0]},
            {'title': 'Подраздел 1.1.5', 'description': 'Правоотношения', 'order': 5, 'section': sections[0]},
        ]
